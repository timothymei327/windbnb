...
const PhotosUploader = ({formValues, setFormValues, handleChange}) => {
  ...
  const setMainPhoto = async (e, fileName) => {
    e.preventDefault()
    setFormValues({...formValues, photos: [fileName, ...photos.filter(photo => photo !== fileName)]})  
  }
        ...
        {formValues.photos.map(link => (
          <div className='flex h-32 relative' key={link}>
            <img
              ...
              <button onClick={e => setMainPhoto(e,link)} className="absolute bottom-1 left-1 text-white bg-black bg-opacity-50 rounded-full p-1">
              {link === photos[0] && (
          ...
          <label className='flex items-center justify-center border border-gray-300 bg-transparent rounded-xl h-32 text-lg text-gray-600 cursor-pointer mb-2'>
